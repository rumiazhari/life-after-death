...
## Current goal
Iteration 9: low-health screen-edge vignette in CombatFeedback — radial
dark-red falloff that closes in continuously as health drops (smooth onset
at 0.6 ratio, peak near critical), complements the existing pulsing
low-health vignette. + 1 headless test (vignette_strength monotonic vs
health, clears on respawn). Iteration 8 is committed (suite 200/0).
...
## Backlog (top-down)
1. Player-visible polish candidates (freedom scope): weather/vignette tied to SimulationClock; footstep/ambient audio loops. Split one per iteration.
