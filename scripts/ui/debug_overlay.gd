...
func _process(delta: float) -> void:
	...
	label.text = ("FPS %d | frame %.2fms | physics %.2fms\n" +
		...
		job_board.jobs_available_count() if job_board else 0,
		job_board.jobs_reserved_count() if job_board else 0,
...
func _collect_survivor_stats(delta: float) -> Dictionary:
	...
	for survivor in get_tree().get_nodes_in_group("survivors"):
		var ai: SurvivorAI = (survivor as Survivor).ai if survivor is Survivor else null
		if ai == null:
			continue
		count += 1
